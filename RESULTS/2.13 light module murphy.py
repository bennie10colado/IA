...
class MurphyReactiveAgent:
    def __init__(self):
        pass

    def choose_action(self, dirty):
        if dirty:
            # Lei de Murphy: falha ao limpar o chão 25% do tempo e deposita sujeira 25% do tempo
            return random.choices(['Suck', 'FailSuck', 'DepositDirt'], weights=[0.5, 0.25, 0.25])[0]
        else:
...
def simulate_simple_reactive_agent(size=(3, 3), dirt_probability=0.2, obstacle_probability=0.1, time_steps=1000):
    ...
    for _ in range(time_steps):
        ...
        if environment[agent_position[0]][agent_position[1]] == 'X':
            ...
            agent = MurphyReactiveAgent()
            action = agent.choose_action(dirty)

            # Sensor de sujeira fornecendo resposta errada 10% do tempo
...
if __name__ == "__main__":
    ...
    for _ in range(num_simulations):
        ...
        print(f"Desempenho do agente reativo simples com obstáculos e Lei de Murphy: {score_simple}")

    average_score_simple = total_scores_simple / num_simulations
    print(f"Média de desempenho do agente reativo simples com obstáculos e Lei de Murphy: {average_score_simple}")
